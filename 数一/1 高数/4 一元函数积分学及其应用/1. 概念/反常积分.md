...
因为[[数一\1 高数\4 一元函数积分学及其应用\1. 概念\定积分|定积分]]有两个必要条件：

1. 积[[数一\1 高数\0 汇总\方法与思维\分区间|分区间]]**有限**；
2. 被积函数**有界**。

破坏第一条“积[[数一\1 高数\0 汇总\方法与思维\分区间|分区间]]有限”，则引出**无穷区间**上的反常积分；

破坏第二条“被积函数有界”，则引出**无界函数**的反常积分。

> [!info] 
> 
> 因为人们发现，无穷区间或者无界函数围成的面积也可能是**有限**的，所以对[[数一\1 高数\4 一元函数积分学及其应用\1. 概念\定积分|定积分]]的概念进行了**推广**。因此定积分和反常积分也有两个别名：
> 
> - *定积分 — 常义积分*
> - *反常积分 — 广义积分*
...
> 所以反常积分收敛。
>
> $$\int^{+\infty}_{-\infty} x^{3} \mathrm{d}x = \int^{0}_{-\infty} x^3 \mathrm{d}x + \int^{+\infty}_{0} x^3 \mathrm{d}x\ne 0$$
> 左右均不收敛，因此反常积分发散。*但是*：
...
> 
> 因此联想到 [[../../1-1. 函数的极限与连续/函数极限的计算#2. 七种未定式的计算]]
...
> [!summary] 
> 1. $\int^{+\infty}_{a} f(x) \mathrm{d}x$ 看 $f(x) \to 0$ 的速度（[[数一\1 高数\1 函数 极限与连续\无穷小|无穷小]]的阶数）。
> 2. $\int^{b}_{a} f(x) \mathrm{d}x$ ，其中 $\lim_{x \to a^{+}}f(x = \infty)$ 看 $f(x) \to \infty$ 的速度（[[数一\1 高数\1 函数 极限与连续\无穷大|无穷大]]的阶数）。

## 敛散性判别

### 无穷区间（$f(x),g(x)$ 趋向于 $0$）

#### [[数一\1 高数\4 一元函数积分学及其应用\比较判别法|比较判别法]]
...
#### [[数一\1 高数\4 一元函数积分学及其应用\比较判别法|比较判别法]]的极限形式 
...
2. $\lambda = 0$ ：若 $\int^{a}_{b}g(x)\mathrm{d}x$ 收敛，则 $\int^{a}_{b}f(x)\mathrm{d}x$ 也收敛；
...
> 0 为瑕点，越小越收敛
> 
> **这个结论本质上只取决于瑕点附近的幂次，不依赖于瑕点是不是 0、区间上限是不是 1**
> 
>
>2. $$\int^{+\infty}_{1} \frac{1}{x^{p}} \mathrm{d}x \begin{cases} \text{ 收敛, }~ p > 1 \\ \text{ 发散, } ~p \le 1 \end{cases}$$
> $+\infty$ 为瑕点，越大越收敛
> 
> #重要 **对于 p 级数**，趋向于 $x=0$ 速度越慢，越靠近 $y$ 轴，越可能收敛；趋向于 $x=+\infty$ 越快，越靠近 $x$ 轴，越可能收敛
> 
> 
> ![liansan](assets/liansan.png)
> ![](assets/反常积分/file-20260421200409909.png)
...
若 $\int^{+\infty}_{a} |f(x)| \mathrm{d}x$ 收敛，则 $\int^{+\infty}_{a} f(x) \mathrm{d}x$ 也收敛
