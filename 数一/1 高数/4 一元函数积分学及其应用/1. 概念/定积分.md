...
当定积分存在，定积分的值只和**函数本身** $f(x)$ 与**积[[数一\1 高数\0 汇总\方法与思维\分区间|分区间]]** $a,b$ 有关，与变量的记法无关。
...
1. 若 $f(x)$ 在**闭区间**上**连续**，则积分存在；
	- 连续函数一定有[[[数一\1 高数\4 一元函数积分学及其应用\1. 概念\不定积分|不定积分]]](1-4.%20一元函数积分学/1.%20概念/不定积分.md#^zje9ul)
2. 若 $f(x)$ 在**闭区间**上**有界**，且只有**有限个间断点**，则积分存在； ^f7k6z9
	- 不能是*无穷间断点*，因为 $f(x)$ 要在闭区间上*有界*
	- 函数存在有限个间断点 (除无穷间断点) 时，所围成图形的面积依然存在，故积分存在
3. 若 $f(x)$ 在**闭区间**上**单调**，则积分存在。

> [!caution]
> 1. 在后面提到 [[反常积分]] （广义积分）时，函数无界也可能积分存在。但在**常义可积**时依然还是**可积必有界**。
> 2. [[数一\1 高数\4 一元函数积分学及其应用\定积分存在定理|定积分存在定理]]与 [[不定积分#不定积分存在定理]] 的区别与联系
...
$$
> [!note] #熟记 
> $\left| \int^{b}_{a} f(x) \mathrm{d}x \right|$ 是"**亡羊补牢**"，内部已经运算结束，在外面加绝对值也没用
> 与[三角不等式](0.%20杂项/[[0. 杂项\常用公式|常用公式]].md#三角不等式)类似 ^0vesv8
...
> [!tip] 
>
> 事实上，只要区间内**非负**的 $f(x)$ **不恒等于零**，那么其积分**一定大于0**。 ^0vi3qw
> 
...
#### 性质6：[[数一\1 高数\2 一元函数微分学及其应用\1-3. 一元函数微分学\3. 应用\2. 中值定理、微分等式与微分不等式\中值定理|中值定理]]

#重要 又称为*[[数一\1 高数\4 一元函数积分学及其应用\积分中值定理|积分中值定理]]*。
