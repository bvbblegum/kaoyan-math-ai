...
- $\Delta x \Delta y$ ：他是比 $\rho = \sqrt{(\Delta x)^{2} + (\Delta y)^{2}}$ 的高阶[[数一\1 高数\1 函数 极限与连续\无穷小|无穷小]]量。即：
...
设函数 $z=f(x,y)$ 在点 $(x,y)$ 的某实心邻域有定义，若该点的全增量 $$\Delta z = f(x + \Delta x, y + \Delta y) - f(x, y)$$ 可表示为 $$\Delta z = A \Delta x + B \Delta y + o(\rho)$$ 其中 $A,B$ 仅与点 $(x,y)$ 有关而与 $\Delta x, \Delta y$ 无关； $\rho = \sqrt{(\Delta x)^{2}+ (\Delta y)^2}$，且当 $\Delta x \to 0,\Delta y \to 0$ 时， $o(\rho)$ 为 $\rho$ 的高阶[[数一\1 高数\1 函数 极限与连续\无穷小|无穷小]]，则称：
...
> - $\Delta y = A \Delta x + o(\Delta x)$ ：也是主要部分+距离 $\Delta x$ 的高阶[[数一\1 高数\1 函数 极限与连续\无穷小|无穷小]]，但是二元的高阶无穷小是两个变量的距离了；
> - 二元因为有两个自变量，所以叫 *全* 微分。
...
> 切线 -> 切平面，各个方向的变化率
> 可以用平面上的点的值代替曲面上的点的值
> 全增量=线性增量+高阶[[数一\1 高数\1 函数 极限与连续\无穷小|无穷小]]（高度差）
...
3. 作极限 $\lim\limits_{\substack{\Delta x \to 0 \\ \Delta y \to 0}} \frac{\Delta z - (A\Delta x+B\Delta y)}{\sqrt{(\Delta x)^{2} + (\Delta y)^{2}}}$，若极限等于 $0$ （是高阶[[数一\1 高数\1 函数 极限与连续\无穷小|无穷小]]），则在该点可微。